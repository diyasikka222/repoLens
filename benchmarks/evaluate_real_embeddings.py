#!/usr/bin/env python3
"""Evaluate RepoLens retrieval with a real embedding provider.

Compares three retrieval strategies against the synthetic evaluation dataset:

1. Lexical baseline  (CodeSearcher)
2. Real semantic      (SemanticSearcher + OpenAI-compatible embeddings)
3. Real hybrid        (HybridSearcher combining lexical + real semantic)

Usage
-----
Set the required environment variables, then run from the project root::

    export REPOLENS_EMBEDDING_API_KEY="sk-..."
    export REPOLENS_EMBEDDING_MODEL="text-embedding-3-small"
    # Optional:
    # export REPOLENS_EMBEDDING_BASE_URL="https://api.openai.com"
    # export REPOLENS_EMBEDDING_DIMENSIONS="1536"

    python benchmarks/evaluate_real_embeddings.py

This script is intentionally NOT collected by pytest. It makes real network
calls and requires valid credentials. It will fail immediately with a clear
message if the required environment variables are not set.
"""
...
# ---------------------------------------------------------------------------
# Validate configuration early and loudly
# ---------------------------------------------------------------------------

def _require_env(name: str) -> str:
    """Return the value of ``name`` or print a clear error and exit."""
    import os
    ...
    if not value:
        ...
        print("Required configuration:", file=sys.stderr)
        print("  REPOLENS_EMBEDDING_API_KEY   — API key for the embedding service", file=sys.stderr)
        print("  REPOLENS_EMBEDDING_MODEL     — model identifier (e.g. text-embedding-3-small)", file=sys.stderr)
        print("", file=sys.stderr)
        print("Optional:", file=sys.stderr)
        print("  REPOLENS_EMBEDDING_BASE_URL      — API base URL (default: https://api.openai.com)", file=sys.stderr)
        print("  REPOLENS_EMBEDDING_DIMENSIONS    — embedding dimensions (model default if unset)", file=sys.stderr)
        print("", file=sys.stderr)
        print("Example:", file=sys.stderr)
        print("  export REPOLENS_EMBEDDING_API_KEY='sk-...'", file=sys.stderr)
        print("  export REPOLENS_EMBEDDING_MODEL='text-embedding-3-small'", file=sys.stderr)
        print("  python benchmarks/evaluate_real_embeddings.py", file=sys.stderr)
        sys.exit(1)
    return value
...
def _build_semantic(root: Path, provider) -> tuple:
    ...
    searcher = SemanticSearcher(root, provider)
    return searcher
...
def _format_report_table(report, label: str) -> dict:
    """Extract metrics from an EvaluationReport into a display dict."""
    return {
        "label": label,
...
def _print_comparison(results: list[dict], config_info: dict) -> None:
    ...
    print()
    print(sep)
    print("  RepoLens Real Embedding Evaluation")
    print(sep)
    print()
    ...
    print(f"  {'API base URL:':<30} {config_info['base_url']}")
    print(f"  {'API key:':<30} {config_info['key_display']}")
    if config_info.get("dimensions"):
        print(f"  {'Dimensions:':<30} {config_info['dimensions']}")
    ...
    # Metrics table
    header = f"  {'Metric':<20} {'LEXICAL BASELINE':>18} {'REAL SEMANTIC':>18} {'REAL HYBRID':>18}"
    print(header)
    print(f"  {'-' * 20} {'-' * 18} {'-' * 18} {'-' * 18}")
...
def main() -> None:
    ...
    # 2. Require embedding configuration — fail immediately if missing
    api_key = _require_env("REPOLENS_EMBEDDING_API_KEY")
    model = _require_env("REPOLENS_EMBEDDING_MODEL")

    import os
    base_url = os.environ.get("REPOLENS_EMBEDDING_BASE_URL", "https://api.openai.com").strip() or "https://api.openai.com"
    dims_raw = os.environ.get("REPOLENS_EMBEDDING_DIMENSIONS", "").strip()
    dimensions = int(dims_raw) if dims_raw else None

    # 3. Build the real embedding provider
    from repolens.embeddings import OpenAIEmbeddingProvider

    print("Initializing real embedding provider...")
    ...
    print("Building semantic searcher (embedding repository files)...")
    t0 = time.perf_counter()
    ...
    sem_metrics = _format_report_table(sem_report, "REAL SEMANTIC")
    hyb_metrics = _format_report_table(hyb_report, "REAL HYBRID")

    config_info = {
        "model": model,
        "base_url": base_url,
        "key_display": _mask_key(api_key),
        "dimensions": dimensions or "(model default)",
    ...
    print(f"  Real semantic     — P@5: {p5_sem:.4f}  R@5: {r5_sem:.4f}  MRR: {mrr_sem:.4f}")
    print(f"  Real hybrid       — P@5: {p5_hyb:.4f}  R@5: {r5_hyb:.4f}  MRR: {mrr_hyb:.4f}")
    print()
